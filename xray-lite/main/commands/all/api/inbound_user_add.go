...
import (
	...
	// Phase XL.2: Removed unused protocol imports for binary size reduction
...
func extractInboundUsers(inb *core.InboundHandlerConfig) []*protocol.User {
	...
	// Phase XL.2: Only vless supported after protocol pruning
	switch ty := inst.(type) {
	case *vlessin.Config:
		return ty.Clients
	default:
		fmt.Println("unsupported inbound type (only vless supported in xray-lite)")
	}
	return nil
}
