...
import (
	...
	// Phase XL.2: Removed http proxy import for binary size reduction
...
// Phase XL.2: Removed httpServer field - HTTP fallback disabled for binary size reduction
type Server struct {
	config        *ServerConfig
...
// Phase XL.2: Simplified - removed HTTP server initialization
...
func (s *Server) Process(ctx context.Context, network net.Network, conn stat.Connection, dispatcher routing.Dispatcher) error {
	...
	case net.Network_TCP:
		...
		if firstbyte[0] != 5 && firstbyte[0] != 4 { // Check if it is Socks5/4/4a
			// Phase XL.2: HTTP fallback removed for binary size reduction
			errors.LogDebug(ctx, "Not SOCKS request (first byte: ", firstbyte[0], "), rejecting connection")
			return errors.New("not a SOCKS request, HTTP fallback disabled")
		}
