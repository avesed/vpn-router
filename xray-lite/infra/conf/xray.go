...
var (
	// Phase XL.2: Pruned to only include used protocols
	// KEPT: vless (primary), dokodemo (stats API), socks (bridge), freedom (direct)
	inboundConfigLoader = NewJSONConfigLoader(ConfigCreatorCache{
		...
		// REMOVED: http, shadowsocks, vmess, trojan, wireguard
	}, "protocol", "settings")
	...
	outboundConfigLoader = NewJSONConfigLoader(ConfigCreatorCache{
		...
		// REMOVED: block/blackhole, loopback, http, shadowsocks, vmess, trojan, dns, wireguard
	}, "protocol", "settings")
)
...
func (c *Config) Build() (*core.Config, error) {
	...
	// RouterConfig is ignored (stub returns error)
	if c.RouterConfig != nil {
		// Skip - routing handled by Rust rule engine
	}

	// Phase XL.4: DNS module removed - sing-box handles DNS
	// DNSConfig is ignored (stub returns error)
	if c.DNSConfig != nil {
		// Skip - DNS handled by sing-box
	}
	...
	if c.Observatory != nil {
		// Skip - health check handled by Python
	}

	// Phase XL.4: BurstObservatory module removed - Python health check handles this
