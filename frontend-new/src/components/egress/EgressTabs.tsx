...
export function EgressTabs() {
  ...
  if (isLoading) return <div>Loading egresses...</div>;
  if (error) return <div>Error loading egresses: {error.message}</div>;
  ...
  const handleDelete = (type: string, tag: string) => {
    ...
    toast.promise(promise, {
      loading: "Deleting egress...",
      success: "Egress deleted successfully",
      error: "Failed to delete egress",
    });
  };

  const renderEgressList = (list: EgressItem[], type: string) => {
    if (!list || list.length === 0) {
      return <div className="text-center p-8 text-muted-foreground">No {type} egresses configured.</div>;
    }
    return (
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold tracking-tight">Egress Management</h2>
        <div className="flex gap-2">
          {activeTab === "custom" && (
            <Button onClick={() => setShowAddWireGuard(true)}>
              <Plus className="mr-2 h-4 w-4" /> Add WireGuard
            </Button>
          )}
          {activeTab === "direct" && (
            <Button onClick={() => setShowAddDirect(true)}>
              <Plus className="mr-2 h-4 w-4" /> Add Direct
            </Button>
          )}
          {activeTab === "warp" && (
            <Button onClick={() => setShowAddWarp(true)}>
              <Plus className="mr-2 h-4 w-4" /> Register WARP
            </Button>
          )}
        </div>
      </div>
