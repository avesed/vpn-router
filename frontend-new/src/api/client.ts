...
async function fetchImageAsBlob(path: string): Promise<string> {
  const response = await fetchWithAuth(path, { headers: { Accept: "image/*" } });
...
export const api = {
  ...
  importPairRequest: (data: ImportPairRequestRequest) =>
    request<ImportPairRequestResponse>("/peer/import-pair-request", {
      method: "POST",
      body: data
    }),

  completePairing: (data: CompletePairingRequest) =>
    request<CompletePairingResponse>("/peer/complete-pairing", {
      method: "POST",
      body: data
    }),

  // Peer Inbound API
  enablePeerInbound: (tag: string, port?: number) =>
    request<EnableInboundResponse>(`/peers/${encodeURIComponent(tag)}/inbound/enable`, {
      method: "POST",
      body: port ? { port } : {}
    }),
