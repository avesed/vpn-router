  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-white">端点配置</h2>
          <p className="mt-1 text-sm text-slate-400">
            查看和调整 WireGuard 底层参数（MTU、密钥等）
          </p>
        </div>
        <button
        ...
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 px-5 py-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-rose-200">加载失败</p>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-slate-400">加载端点配置中...</p>
          </div>
        </div>
      ) : endpoints.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-white/5 bg-slate-900/40 py-16">
          ...
          <p className="text-lg font-semibold text-slate-300">未找到端点配置</p>
          <p className="mt-1 text-sm text-slate-500">请检查 sing-box 配置</p>
        </div>
      ) : (
      ...
      {!loading && endpoints.length > 0 && (
        <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4">
          <p className="text-xs text-blue-300">
            修改端点配置后，建议重启 sing-box 服务以使更改生效
          </p>
        </div>
      )}
    </div>
