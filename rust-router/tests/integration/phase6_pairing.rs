...
use rust_router::peer::{
    decode_pair_request, HealthChecker, PairRequest, PairRequestConfig, PairResponse, PeerManager,
    TunnelIpAllocator, TunnelPortAllocator,
};
...
async fn test_pairing_bidirectional() {
    ...
    let response_result = manager_b
        ...
        .await
        .expect("Should import pairing request");
    ...
    let peer_a_on_b = manager_b
        .get_peer_config("node-a")
        .expect("Node A should exist on B");
    let peer_b_on_a = manager_a
        .get_peer_config("node-b")
        .expect("Node B should exist on A");
...
fn test_xray_pair_request_fields() {
    ...
    assert_eq!(
        decoded.xray_public_key,
        Some("some-reality-public-key".to_string())
    );
