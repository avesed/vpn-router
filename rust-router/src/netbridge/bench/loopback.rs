//! Loopback test - Pure in-memory performance testing
//!
...
//! simulate a remote server. This allows benchmarking pure netbridge performance
...
//! ├─────────────────────────────────────────────────────────────────────┤
//! │                                                                     │
//! │  Test Client                   MockResponder                        │
//! │       │                              ▲                              │
//! │       │ write data                   │ reply packet                 │
//! │       ▼                              │                              │
//! │  SmoltcpEgress                       │                              │
//! │       │                              │                              │
//! │       │ TCP/UDP → IP                 │ swap addrs                   │
//! │       │ packets                      │                              │
//! │       ▼                              │                              │
//! │   drain_tx() ───────────────────────►│                              │
//! │       │                              │                              │
...
impl MockResponder {
    ...
    pub fn process_packet(&self, packet: &[u8]) -> Option<Vec<u8>> {
        ...
        self.bytes_processed.fetch_add(packet.len() as u64, Ordering::Relaxed);
...
impl LoopbackTest {
    ...
    pub async fn run_throughput(&self, bench_config: &BenchConfig) -> BenchResults {
        ...
        let mut results = match bench_config.pattern {
            TrafficPattern::SingleTcp => {
                self.run_single_tcp_throughput(bench_config).await
            }
            TrafficPattern::ManyShortTcp => {
                self.run_many_tcp_throughput(bench_config).await
            }
            TrafficPattern::UdpStream => {
                self.run_udp_throughput(bench_config).await
            }
            TrafficPattern::DnsLikeUdp => {
                self.run_dns_like_udp(bench_config).await
            }
            TrafficPattern::Mixed => {
                self.run_mixed_throughput(bench_config).await
            }
        };
    ...
    pub async fn run_latency(&self, bench_config: &BenchConfig) -> BenchResults {
        ...
        let responder = Arc::new(MockResponder::new(true));
        ...
        let dest = SocketAddr::new(
            IpAddr::V4(self.config.remote_ip),
        ...
        for _ in 0..iterations {
            ...
            let src = SocketAddr::new(
                IpAddr::V4(Ipv4Addr::new(CLIENT_IP[0], CLIENT_IP[1], CLIENT_IP[2], CLIENT_IP[3])),
                12345,
            );
    ...
    async fn run_single_tcp_throughput(&self, bench_config: &BenchConfig) -> BenchResults {
        let egress_config = SmoltcpEgressConfig::new(self.config.local_ip);
        let (egress, handle) = SmoltcpEgress::spawn(egress_config);
        let responder = Arc::new(MockResponder::new(self.config.echo_mode));
        let shutdown = Arc::new(AtomicBool::new(false));
        ...
        let send_task = tokio::spawn(async move {
            while start.elapsed() < duration {
                if server_tx.send(Bytes::from(data_block.clone())).await.is_err() {
                    break;
                }
